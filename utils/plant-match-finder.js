...
class PlantMatchFinder {
   ...
   * 使用递归查找算法（借鉴小蚂蚁教程）
   * 
   * @param {Object} startPlant 起始植物
   * @returns {Object} 匹配结果
   */
  findMatches(startPlant) {
    this.visited.clear();
    const matches = [];
   ...
   * @returns {Object|null} 相邻植物
   */
  getAdjacentPlant(plant, rowOffset, colOffset) {
  ...
  /**
   * 执行消除操作
   * @param {Array} matches 匹配的植物列表
   * @returns {number} 消除的植物数量
   */
  executeElimination(matches) {
   ...
   * 检查滑动操作是否有效
   * @param {Object} startPlant 起始植物
   * @param {Object} endPlant 结束植物
   * @returns {boolean} 是否有效
   */
  isValidSwipe(startPlant, endPlant) {
    // 必须是相同类型
...
module.exports = PlantMatchFinder;
